...
- **Fastify API (`apps/api`):** использует встроенные возможности PostgreSQL (`pg_trgm` для поиска похожих, табличные правила классификации для быстрого отклика).
- **Python AI ядро (`ai/`):** реализует расширенный ML/NLP пайплайн (TurkicNLP + Natasha, семантический векторный поиск, KazLLM / Qwen fallback, Erlang-C).
- **Роль (Руководитель/Оператор):** переключение через UI для демонстрации разных профилей работы.
- **Данные:** исторический срез по 7 регионам (~1 млн обращений).
